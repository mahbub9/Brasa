 ...
 *
 * Deliberately the same cookie name as pos's own languageStorage.ts
 * (`brasa.lang`, no `Domain` attribute, so scoped to the host but not the
 * port — shared across `pos` and `admin` on `localhost` in dev). A staff
 * member's language choice is a preference for *them*, not for whichever
 * client app they happened to open first.
 */
 ...
 * it is never sent to or read by the API. It does not conflict with ADR 0008
 * (token auth, no cookies): that ADR is about authentication, not about
 * where a browser happens to keep a client-only rendering preference.
 */
