...
| `Endpoints/FloorEndpoints.cs` | ✅ | `GET /floor`, `POST /tables/{id}/clear`, `POST /tables/{id}/request-bill` (FLR-04 — mirrors `ClearTableAsync` exactly, incl. the `DbUpdateConcurrencyException` → same-guard-error handling for the `xmin` race) |
...
| File | State | Contents |
|---|---|---|
| `src/App.tsx` | ✅ | Section switcher (`overview`/`menu`, local state — no router yet) plus "Visão geral"/"Overview": real category/item/room/table counts from `GET /menu/all`/`GET /floor`. Floor plan/Staff nav entries still render "Brevemente"/"Coming soon" |
...
| `tests/admin-shell.spec.ts` | ✅ | WEB-09 — loads `admin` (port 5174) and checks "Visão geral" shows real, non-zero counts from `GET /menu/all`/`GET /floor` (not a static mock), and that Floor plan/Staff are labelled "Brevemente" rather than silently missing while Menu (live since WEB-10) is not |
