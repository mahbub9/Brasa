...
| `src/components/MenuManager.tsx` | ✅ | WEB-10 — per category: visibility toggle; per item: availability toggle, inline price edit, two-step delete confirm, inline takeaway-price add/edit/clear (CAT-06, next to the dine-in price field), inline schedule add/edit/clear (CAT-11 — seven day checkboxes plus native `<input type="time">` start/end fields, whose `"HH:mm"` value needs no conversion to match the wire format). A CSV file input reusing CAT-17's existing import pipeline is the *only* way to add a new item — there's no "create item" endpoint, admin or otherwise. Every mutation refetches `GET /menu/all` rather than reconciling local state by hand |
