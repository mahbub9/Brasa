...
|---|---|---|
| `Identity` | 📁 empty | I3 |
| `Catalog` | ✅ `MenuCategory`, `MenuItem` (soft-deletable, CAT-18), `ModifierGroup`/`Modifier` (CAT-03/04, one item per group for now), `VatRate` (I0 placeholder for I1's full `TaxRule`), `Course` (CAT-14 — which point in the meal an item is served at, independent of `MenuCategory`), EF config + migrations + design-time factory | — |
