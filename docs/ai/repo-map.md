...
**Last verified:** 2026-08-10 · Companion to [README.md](README.md)
...
| `Endpoints/FloorEndpoints.cs` | ✅ | `GET /floor`, `POST /tables/{id}/clear`, `POST /tables/{id}/request-bill` (FLR-04 — mirrors `ClearTableAsync` exactly, incl. the `DbUpdateConcurrencyException` → same-guard-error handling for the `xmin` race), `POST /rooms/{id}/tables`/`PUT /tables/{id}`/`DELETE /tables/{id}` (FLR-03 table CRUD, not the drag-and-drop canvas that row's own title names; `DeleteTableAsync` requires `Free`, `UpdateTableAsync`'s concurrency race is a genuine lost-update, not a state re-check, so it gets its own code — `floor.table_concurrently_modified` — rather than reusing one of the state-specific ones), `POST /rooms`/`PUT /rooms/{id}`/`DELETE /rooms/{id}` (FLR-03's room-CRUD follow-up — `DeleteRoomAsync` checks `db.Tables.AnyAsync` directly, since `Room` has no navigation to `Table` to check from the domain side) |
...
| `Floor` | ✅ `Room` (`Update` — FLR-03's room-CRUD follow-up, renaming/reordering), `Table` (`Free`/`Occupied`/`BillRequested`/`Dirty` state machine, `xmin` optimistic concurrency — see the trap in [README.md](README.md); `Update`/`EnsureCanDelete` — FLR-03's table CRUD, editing/removing a table); EF config + migrations (RLS) + design-time factory | — |
