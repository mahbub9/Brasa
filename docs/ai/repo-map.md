...
| `Endpoints/FloorEndpoints.cs` | ✅ | `GET /floor`, `POST /tables/{id}/clear`, `POST /tables/{id}/request-bill` (FLR-04 — mirrors `ClearTableAsync` exactly, incl. the `DbUpdateConcurrencyException` → same-guard-error handling for the `xmin` race), `POST /rooms/{id}/tables`/`PUT /tables/{id}`/`DELETE /tables/{id}` (FLR-03's first slice — table CRUD, not the drag-and-drop canvas that row's own title names; `DeleteTableAsync` requires `Free`, `UpdateTableAsync`'s concurrency race is a genuine lost-update, not a state re-check, so it gets its own code — `floor.table_concurrently_modified` — rather than reusing one of the state-specific ones) |
...
| `Floor` | ✅ `Room`, `Table` (`Free`/`Occupied`/`BillRequested`/`Dirty` state machine, `xmin` optimistic concurrency — see the trap in [README.md](README.md); `Update`/`EnsureCanDelete` — FLR-03's first slice, editing/removing a table); EF config + migrations (RLS) + design-time factory | — |
...
| `src/components/FloorManager.tsx` | ✅ | FLR-03's first slice — per room: table list with inline edit (label/seats/shape/position, plain inputs, no drag canvas) and delete (two-step confirm, disabled unless the table is `Free`); an "add table" form per room. No "create room" form — rooms stay seeded-only (FLR-01), same real gap at the API layer as WEB-10's missing "create category"/"create item" |
...
| `tests/floor-table-management.spec.ts` | ✅ | FLR-03's first slice — creates/edits/deletes a table via the API, round-tripping through `GET /floor`; an empty label, zero seats, an unrecognised shape and an unknown room all rejected on create; an unknown table 404s on edit/delete; deleting a non-`Free` table 409s (`floor.table_not_free`); the `admin` UI round trip (add → edit → delete) works in a real browser |
