...
| `tests/menu-category-visibility.spec.ts` | ✅ | CAT-01 — hides the real seeded "Sobremesas" category (unreferenced by any other spec, so safe without an isolated fixture — there's no endpoint to create a category at all) and confirms it and both its items vanish from `GET /menu`; showing it restores both; unknown category `404`s |
...
| `tests/admin-menu-management.spec.ts` | ✅ | WEB-10 — API-level: `GET /menu/all` includes a hidden category and an 86'd item that `GET /menu` excludes, correctly flagged. UI-level: toggling a category's visibility, 86-ing and repricing an item, and CSV-importing a new one all round-trip through the real `admin` UI to the API and back, using freshly-imported/uniquely-named test items (same isolation convention as `menu-availability.spec.ts`/`menu-price.spec.ts`) or the real "Sobremesas" category (same convention as `menu-category-visibility.spec.ts`) so nothing collides with sibling specs under parallel workers |
