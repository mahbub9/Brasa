...
- ✅ **Built, tested, and proven live** (not just unit-tested — see §3a):
  ...
  **real RLS** (DAT-01…06),
  ...
  UI (`src/web/e2e`, QA-01/03/05/14 incl. axe-core accessibility scans, 59
  ...
  endpoint to reach them, the same shape as FLR-04), a committed OpenAPI
...
- **Unused-looking code isn't necessarily dead — check for a domain method
  or guard with no caller before assuming something is finished.** Two
  ...
  `PUT /menu/items/{id}/availability` closed that one. Before deleting
  UI/CSS/i18n/a domain method that looks unreferenced, check whether it's
  scaffolding for a state or guard that already exists and is already
  enforced, just missing the one endpoint that would reach it — the same
  way CAT-02's fields shipped ahead of the UI that would set them.
