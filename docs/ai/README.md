...
- ✅ **Built, tested, and proven live** (not just unit-tested — see §3a):
  ...
  **real RLS** (DAT-01…06),
  ...
  UI (`src/web/e2e`, QA-01/03/05/14 incl. axe-core accessibility scans, 57
  ...
  calls the endpoint yet), cursor pagination on `GET /orders` (API-09 —
  ...
  endpoint did, see §7), a committed OpenAPI document ([docs/openapi/v1.json](../openapi/v1.json),
...
- **Unused-looking frontend code isn't necessarily dead.** `TablePicker.tsx`'s
  `.floor-table-BillRequested` CSS and the `floor.state.BillRequested` i18n
  strings existed for a full state — `Table.RequestBill()` — that had *no*
  producer anywhere: no endpoint called it, so the UI could render a state
  that could never actually occur. FLR-04 added `POST /tables/{id}/request-bill`
  and a "Pedir conta" button to finally reach it. Before deleting
  UI/CSS/i18n that looks unreferenced, check whether it's future-facing
  scaffolding for a domain state that already exists, the same way CAT-02's
  fields shipped ahead of the UI that would set them.
