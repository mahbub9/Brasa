...
| `order.close_conflict_after_fiscal_issuance` | Conflict | 409 | `POST /orders/{id}/close`'s (ORD-21) `xmin` concurrency token was stale — someone else changed this order between the read and the write — but only discovered *after* `IFiscalProvider.IssueSimplifiedInvoiceAsync` had already issued a real, un-un-issuable document (hard rule 3). Distinct from `order.concurrently_modified` on purpose: that one is always safe to retry, this one is not — a naive retry would call the fiscal provider a second time for what the caller believes is still one close attempt. Requires a human to reconcile the order against the fiscal document already on file, not an automatic retry. |
| `order.concurrently_modified` | Conflict | 409 | Any other order-mutating endpoint's (ORD-21) `xmin` concurrency token was stale — someone else changed the same order between the read and the write, the exact race `floor.table_concurrently_modified` already catches for a table. Safe to retry: nothing external has happened yet at any of these call sites. |
...
| `cash_session.not_found` | NotFound | 404 | `POST /cash-sessions/{id}/close`, `GET /cash-sessions/{id}`, `POST /cash-sessions/{id}/movements`, `GET /cash-sessions/{id}/movements`, `POST /cash-sessions/{id}/count` or `GET /cash-sessions/{id}/variance`'s (PAY-08/09/10/11) session id doesn't exist. Also returned by `POST /orders/{id}/payments` or `POST /orders/{id}/payments/split` (PAY-11) when a request's optional `cashSessionId` names an unknown session. |
