...
| CI (GitHub Actions) | ✅ | Runs on every push to `main`. Build-and-test, `openapi-drift` (API-14) and vulnerable-package-scan jobs green. `openapi-drift` caught real drift three times running (CAT-02, ORD-07/08/09, CAT-17 each shipped an endpoint without the documented regeneration step) — fixed by regenerating the doc and by adding `infra/scripts/verify.ps1` to catch this locally before a commit, not only after a push. The `e2e` job's earlier, previously-undiagnosed red runs (no `gh` CLI/token available in this environment to pull job logs, so it stayed an open gap for a while) very likely trace to a real bug found while writing `floor-drag-drop.spec.ts`: `IdempotencyMiddleware` crashed the whole API process on any `204 No Content` response (any DELETE endpoint), which a busy full-suite CI run was always going to hit sooner or later, cascading into failures on whatever ran next — not a flake, and not specific to this environment. Fixed; see the `IdempotencyMiddleware` entry in [docs/ai/README.md](../ai/README.md) for the mechanism and [E2E (Playwright)](#tests) above for the regression test |
