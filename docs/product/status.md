...
| `Modules.Payments` | 🚧 | `Payment` (PAY-01/02/05) — a cash tender recorded against an order's remaining balance (`OrderId`, `Method` — only `Cash` exists today, `AmountDue`/`AmountTendered` snapshotted, computed `AmountApplied`/`Change`/`RemainingBalance`, `PaidAtUtc`), own `payments` schema/`PaymentsDbContext`. `POST /orders/{id}/payments` computes what's still owed server-side (the order's total minus every prior payment's `AmountApplied`) and computes change; a tender smaller than that is a valid partial payment, `400 payment.already_settled` once nothing remains owed. `GET /orders/{id}/payments` lists them. Purely additive — does not gate `Order.Close()`. Split tender across several *methods* at once (PAY-04) still unbuilt — only `Cash` exists. Cash session/blind-count/*fecho de caixa* (PAY-08/10/11) still unbuilt. See [cash payments](../features/cash-payments.md) |
