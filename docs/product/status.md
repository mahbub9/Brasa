...
| `kds` | ⬜ | |
| `admin` | 🚧 WEB-09 shell | React 19 + Vite 8 + TS, same tooling as `pos`, on port 5174. One live screen ("Visão geral"/"Overview"): real category/item/room/table counts from `GET /menu` and `GET /floor`, so the shell is provably wired to the API and not a static mock. Menu/Floor/Staff nav entries are labelled "Brevemente"/"Coming soon" rather than silently missing — WEB-10/11 build the actual editors. pt-PT default / en toggle (ADR 0011, shares `pos`'s `brasa.lang` cookie) with real English copy throughout — not all staff read Portuguese. No auth yet (depends on IDN) |
