...
| `Modules.Payments` | 🚧 | `Payment` (PAY-01/02) — a cash tender recorded against an order (`OrderId`, `Method` — only `Cash` exists today, `AmountDue`/`AmountTendered` snapshotted, computed `Change`, `PaidAtUtc`), own `payments` schema/`PaymentsDbContext`. `POST /orders/{id}/payments` reads the order's own total server-side and computes change; `GET /orders/{id}/payments` lists them. Purely additive — does not gate `Order.Close()`, and no partial/split tender yet (PAY-04/05). Cash session/blind-count/*fecho de caixa* (PAY-08/10/11) still unbuilt. See [cash payments](../features/cash-payments.md) |
