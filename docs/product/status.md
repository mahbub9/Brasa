...
**Last updated:** 2026-08-16 · **Roadmap phase:** I0 complete except deployment (OPS-11); I1's opening slice (real rooms and tables, FLR) proven end-to-end
...
| Privileged system-context role (DAT-07) | ✅ | `brasa_system` (`infra/initdb/02-system-role.sql`) — the "third, narrowly-scoped role" `docs/architecture/multi-tenancy.md`'s own system-context section once deferred, not a superuser/`BYPASSRLS` shortcut. Read-only, cross-tenant: `RowLevelSecurity.EnableSystemReadFor` grants a role-scoped `FOR SELECT ... USING (true)` policy per table, enforced twice (policy + a `GRANT` that never includes write verbs) so a bug in the policy expression alone could never turn this into a write path. `ModulePersistenceExtensions` selects a physically separate connection string (`ConnectionStrings:PostgresSystem`) whenever `ITenantContext.IsSystemContext` is set, so an ordinary request can never end up on a connection still carrying the elevated role — a structural guarantee, not a connection-pool-reset one. No real consumer yet (the SAF-T sweep, FIS-23, is unbuilt) — mechanism only. **Verified live**: `SystemContextIntegrationTests.cs` |
| `Database:Provider` toggle | ✅ | `Postgres` (default, required in Production) or `InMemory` — a single config flag swapping every module's `DbContext`, Hangfire's storage and the readiness check, no PostgreSQL install needed for the pilot beta. `Database:SeedOnStartup` separately gates the dev seeders so a beta restart doesn't reseed demo data over what a restaurant entered live. See [ADR 0012](../architecture/decisions/0012-beta-in-memory-database.md) — RLS has no InMemory equivalent, an accepted trade-down for this one-tenant pilot only |
| `Modules.Identity` | 🚧 | IDN-01 — `Organization`/`Site`/`Terminal` registry (create + list, no update/delete yet), `Site.Region` a real `PortugueseRegion` from day one. `Staff` (IDN-08/09) — site-scoped PIN accounts, PBKDF2 hashing, 5-strike/15-minute lockout, admin-driven rotation; verifies a *known* staff id's PIN, not "identify me by PIN alone." `Staff.VerifyPin` is now also what gates a privileged Ordering action (IDN-11) — `OrderEndpoints` composes `IdentityDbContext` in, the same Catalog+Identity composition shape `PriceListEndpoints` already uses. OAuth/JWT sessions, device/terminal pairing, roles-and-permissions beyond a bare Staff/Manager tag (the rest of this epic) still I3 |
| `Modules.Catalog` | ✅ | `MenuCategory`, `MenuItem` (incl. optional `Description` and declared `Allergens` — CAT-02; `ImageUrl` — CAT-02, a path into `MenuItemImageStorage`'s local-disk store, never the file bytes), seeded demo menu spanning both VAT bands, soft delete (CAT-18), modifier groups (CAT-03/04), `PriceList`/`PriceListEntry` (CAT-05 — per-site price overrides, create/read/add-entry only, not yet resolved through `AddLine`), `Combo`/`ComboComponent` (CAT-10 — fixed-price bundles; ringing one up *does* reach `AddLine`, decomposing into ordinary lines at allocated prices), scheduled price changes (CAT-16 — `MenuItem.EffectivePrice`, resolved lazily on every read, no background job; `GetMenuAsync` and `AddLine` both resolve through it), `TaxRule` (CAT-07/08 — effective-dated VAT rate by alcohol band × channel × region, create/list/resolve only, not yet wired into `AddLine`/the fiscal document builder) |
| `Modules.Ordering` | ✅ | `Order` aggregate — open against a real `Table` (`TableId`) or as a takeaway with no table at all (ORD-20, `IsTakeaway`), add line with modifiers (price/VAT/modifier snapshot, ORD-05), per-line kitchen notes (ORD-06), split evenly, by item or by cover (ORD-15/16/17), pre-bill preview (ORD-18/19), transfer to a different table (ORD-12, converts a takeaway to dine-in), move a single line onto a different order (ORD-13) or merge two orders (ORD-14, `OrderStatus.Merged`), close, history/search (ORD-22), void a line (ORD-10) and line/order discounts (ORD-11) — both now gated by real manager authorisation (IDN-11), enforced by the caller before either domain method is ever reached; fire unfired lines to the kitchen, a course at a time or all at once (ORD-07/08/09, `FireLines`) — `OrderLine.Course`/`IsFired`/`FiredAtUtc`, no manager authorisation needed since firing touches no money; `Order` now also carries an `xmin` optimistic-concurrency token (ORD-21, the same mechanism `Table` already had) — every mutating endpoint reports a lost race as `409 order.concurrently_modified` instead of a silent last-write-wins overwrite, except `CloseOrderAsync`, whose own distinct `order.close_conflict_after_fiscal_issuance` exists because a lost race there can only be discovered after a real fiscal document was already issued |
| `Modules.Floor` | ✅ | `Room`, `Table` — full `Free ⇄ Occupied ⇄ Dirty ⇄ Free` lifecycle plus `BillRequested` (FLR-04, `POST /tables/{id}/request-bill`), and `Release()` — `Occupied`/`BillRequested` straight back to `Free`, skipping `Dirty`, used only for table transfers (ORD-12). `xmin`-based optimistic concurrency on `Table` so two concurrent occupy attempts can't both win. `Table.GroupId` (FLR-05) pushes 2+ free tables into one seating unit via `POST`/`DELETE /table-groups`; `Occupy()` itself refuses a grouped table rather than leaving grouping purely cosmetic. `Room.FloorLevel` (FLR-07, default `0`) — which physical storey a room sits on; a display-only concern, not an access one. `Room.AssignedStaffId` (FLR-06) — which waiter is working a room as their section, a plain opaque reference to Identity's `Staff` resolved to a name fresh on every `GET /floor`, not snapshotted; any staff role, not manager-only. Seeded: 2 rooms (both floor `0`), 32 tables |
| `Modules.Fiscal` | ✅ | `IFiscalProvider`, `FiscalDocument`, VAT correctly derived from gross (menu prices are VAT-inclusive) |
| `Modules.Payments` | 🚧 | `Payment` (PAY-01/02/03/04/05/06) — a cash or card tender recorded against an order's remaining balance (`OrderId`, `Method` — `Cash`/`Card`, `AmountDue`/`AmountTendered` snapshotted, computed `AmountApplied`/`Change`/`RemainingBalance`, `TipAmount`/`AttributedStaffId`, `PaidAtUtc`), own `payments` schema/`PaymentsDbContext`. `POST /orders/{id}/payments` computes what's still owed server-side (the order's total minus every prior payment's `AmountApplied`) and computes change; a tender smaller than that is a valid partial payment, `400 payment.already_settled` once nothing remains owed. A card tender (PAY-03) can never overpay — `400 payment.card_tender_exceeds_balance` — since a standalone TPA has no change to give back; a card tender smaller than the balance is still a valid partial payment, same as cash. `POST /orders/{id}/payments/split` (PAY-04) records several tenders across any mix of methods as one atomic action — nothing is added to `PaymentsDbContext` until every tender in the batch validates, so a single `SaveChangesAsync` either persists every row or none; a shared `BuildTenderAsync` helper applies the exact same rules as the single-tender endpoint so the two can't drift apart. A tip rides along on an ordinary single tender only, entirely separate from the balance, optionally attributed to a real `Staff` (`404 identity.staff_not_found` if not, `400 payment.invalid_tip_amount` if negative) — `GET /orders/{id}/payments` batch-resolves attributed names the same way `RoomDto.AssignedStaffName` does. Purely additive — does not gate `Order.Close()`. `CashSession` (PAY-08) — a staff member's starting float declaration against a `Terminal`, own `cash_sessions` table in the same schema — `POST /cash-sessions` opens one (`409 cash_session.already_open` if that terminal already has one), `POST /cash-sessions/{id}/close` is a bare status flip, `GET /terminals/{id}/cash-sessions/current` resolves the open one (`204` if none). `CashMovement` (PAY-09) — a pay-in/pay-out against an open session, always with a reason, own `cash_movements` table — `POST /cash-sessions/{id}/movements` (`400 cash_movement.session_closed` if the session isn't open), `GET /cash-sessions/{id}/movements` lists them. A blind cash count (PAY-10) is three nullable fields on `CashSession` itself (`CountedAmount`/`CountedByStaffId`/`CountedAtUtc`) — `POST /cash-sessions/{id}/count` records one, at most once per session (`400 cash_session.already_counted` on a second attempt). *Fecho de caixa* variance reporting (PAY-11) closes the loop: `Payment` gained an optional, nullable `CashSessionId` (an ordinary indexed foreign key, not an opaque reference — `CashSession` already lives in this same module) threaded through both payment endpoints (`404 cash_session.not_found` if given but unresolvable; the split endpoint takes one id for the whole batch, not per-tender), and `GET /cash-sessions/{id}/variance` computes — never stores — the expected drawer amount (float + pay-ins − pay-outs + cash payments taken under that session, card tenders excluded) and, once a count exists, the variance against it; both stay `null` until a count is recorded, and the report stays computable after the session closes. `admin` surfaces it automatically under the blind-count panel the moment a count exists. See [cash sessions](../features/cash-sessions.md) |
...
| Client | State | Notes |
|---|---|---|
| `pos` | ✅ I0/I1 shell | React 19 + Vite 8 + TS: floor table picker (WEB-05, incl. "Nova venda ao balcão" for a takeaway order with no table — ORD-20), grouped by floor once rooms span more than one level (FLR-07) → menu, showing description + allergen tags when declared (CAT-02), incl. a modifier picker for items with groups (CAT-03/04) → lines, each with an inline kitchen-note editor (ORD-06) and a "Sent" badge once fired (ORD-07/08) — a fire-controls bar above the line list offers one button per course still pending plus a "Fire all" → transfer to a different table (ORD-12) → split preview → pre-bill preview (ORD-18/19, clearly labelled *documento não fiscal*) or flag the table `BillRequested` for the floor plan (FLR-04, "Pedir conta" — distinct from previewing the bill) → close → receipt. pt-PT default / en toggle, cookie-persisted (ADR 0011), including seeded table labels ("Mesa 1" → "Table 1" — real staff here aren't all Portuguese speakers) and a language-appropriate takeaway default label. Subscribes to `FloorHub` (API-16, `@microsoft/signalr`) so another terminal opening/clearing/transferring a table shows up live, no manual refresh or poll. Header now has a real staff PIN sign-in too (WEB-07) — a "Sign in" button, staff picker, PIN entry, sign-out — deliberately non-blocking, gating nothing else here yet. Fully restyled through the shared `src/web/ui` design system (WEB-02's own beta UI refresh — real tokens for spacing/radius/elevation/type-scale on top of the existing terracotta identity, 44px touch targets, real hover/focus/disabled states): every component (`StaffLogin`, `TablePicker`, `OrderSummary`, `ModifierPicker`, `PreBill`, `TransferTablePicker`, `Receipt`, `ErrorBanner`, `MenuGrid`) now renders through `Button`/`Badge`/`TextField`/`SelectField`/`Modal`/`Icon` rather than each screen's own hand-rolled CSS (`Card` exists in the shared library too but has no consumer yet in either client). No auth, no offline, no Dexie yet — those are I2 (see [roadmap.md](roadmap.md)) |
| `kds` | ⬜ | |
| `admin` | 🚧 WEB-09 shell + WEB-10 menu/floor editors + WEB-11's staff half + IDN-16 feature flags | React 19 + Vite 8 + TS, same tooling as `pos`, on port 5174. "Visão geral"/"Overview": real category/item/room/table counts from `GET /menu/all` and `GET /floor`, so the shell is provably wired to the API and not a static mock. "Menu": toggle a category's visibility, 86/reprice/delete an item, bulk-import more via CAT-17's CSV pipeline — no "create item" form exists because no such endpoint does either, admin included. "Equipa"/"Staff" (IDN-08/09, WEB-11): list staff, add a staff member with an initial PIN, reset a PIN — no placeholder nav entries left in this shell at all now. "Plano de sala" also gets a section dropdown per room now (FLR-06) — assign or clear which waiter is working it — and a drag-and-drop canvas (FLR-03) alongside the plain add/edit/delete forms: a mouse-only, `aria-hidden` convenience layer over the same `PUT /tables/{id}` the forms already used, since everything it does stays reachable through the accessible form. "Feature flags" (IDN-16, new): add a flag, see every key grouped with its platform overrides, toggle one on or off — no consumer reads a flag anywhere in this codebase yet, ships the mechanism only. Assumes the first organization's first site (no site-selector exists anywhere in either client yet, the same single-site shortcut every other screen here already takes; feature flags are the one screen that needs no site id at all, being tenant- not site-scoped). pt-PT default / en toggle (ADR 0011, shares `pos`'s `brasa.lang` cookie) with real English copy throughout, including seeded table labels on the floor screen ("Mesa 1" → "Table 1", mirroring `pos`'s own `tableLabel.ts`) — not all staff read Portuguese. No auth yet (depends on IDN). Closes WEB-02's own "tokens loaded, components not yet wired in" gap: all four editors (`MenuManager`, `FloorManager`, `StaffManager`, `FeatureFlagManager`) now render their buttons/badges/text-and-select inputs through the same `Button`/`Badge`/`TextField`/`SelectField` components `pos` already adopted, not hand-rolled markup that merely inherited the shared color tokens. `FloorManager`'s pointer-driven drag-and-drop canvas (FLR-03) and every raw click-to-rename tile (a table or room's own name/label, the same "big custom tile" pattern `pos`'s own table/menu tiles already use) deliberately stay outside this — no shared component fits a hand-tracked grid or an unstyled heading-sized hit target. `App.css` trimmed accordingly: the old `.badge-on`/`.badge-off`/`.badge-neutral` pill styling and every hand-rolled `input`/`select` border-and-padding override are gone now that the shared components supply their own. **Verified live**: the full existing admin E2E coverage (`admin-shell`, `admin-menu-management`, `admin-language-toggle`, `staff`, `feature-flags`, `floor-table-management`, `floor-drag-drop`, `floor-multi-level`, `floor-section-assignment`, `menu-import-excel`, `menu-item-image`, `menu-item-couvert`, `menu-item-takeaway-price`) re-run clean with zero spec changes — a pure restyle, not a behaviour change |
...
| `Brasa.Api.IntegrationTests` | ✅ | 37 tests: `TenantIsolationReflectionTests` (DAT-11, no DB) + `TenantIsolationIntegrationTests` (QA-09/10) — real disposable PostgreSQL via Testcontainers, zero rows with no/wrong tenant, own rows only with the right one, DDL refused (the automated version of the manual check that first caught [ADR 0010](../architecture/decisions/0010-rls-runtime-role-split.md)) — plus `CsvParserTests` (CAT-17, no DB), `ErrorMappingTests` (no DB, pins all 6 `ErrorType`→HTTP status mappings directly, since `ErrorMapping.ToProblem()` is, by its own doc comment, "the only place `ErrorType` is translated to an HTTP status"), `ApiDeprecationMiddlewareTests` (API-08, no DB) and `ApiRateLimitingTests` (API-12, no DB — partition-key logic; the real 429/`Retry-After` behaviour was verified live instead, see below); and `OrderConcurrencyIntegrationTests` (ORD-21) — two directly-controlled `OrderingDbContext`s against a real Testcontainers Postgres deterministically prove a lost `xmin` race on `Order` throws `DbUpdateConcurrencyException`, no timing dependence at all, unlike the real-HTTP E2E coverage of the same feature (see below). Adding this test class surfaced a real bug in the harness itself, not the app: it and `TenantIsolationIntegrationTests` both migrate through the same process-wide `BRASA_MIGRATIONS_CONNECTION` env var, and xUnit running them in parallel let one test's migration silently target the other's container — fixed with a shared xUnit `[Collection]` (`MigrationsEnvVarCollection.cs`) forcing the two to run sequentially; and `SystemContextIntegrationTests` (DAT-07) — `brasa_system` sees every tenant with no session variable set at all, `brasa_app` stays exactly as isolated as before it existed, a write and a DDL statement both rejected with `insufficient_privilege`, and the real `ResolveAsSystem()` → EF path proven end to end including the non-obvious `IgnoreQueryFilters()` requirement. Joining `MigrationsEnvVarCollection` immediately hit the same env-var race a third time — fixed for good this time with a shared `TestRoles.EnsureAppAndSystemRolesExistAsync` helper, since two test classes had each been hand-rolling their own `brasa_app`-only role setup and both started failing with "role brasa_system does not exist" the moment a module's real migrations began granting to it too |
| E2E (Playwright) | ✅ | `src/web/e2e` — 224 tests (incl. `cash-sessions.spec.ts` (PAY-08/09/10/11 — opening a session then `GET current` returning it, a duplicate open on the same terminal rejected with `cash_session.already_open`, closing then `GET current` returning `204` then reopening successfully, a negative float/unknown terminal/unknown staff id each rejected with their own code, 404s on an unknown session id and terminal id, a pay-out then a pay-in recorded against an open session and listed oldest-first with names resolved, a closed-session/invalid-direction/non-positive-amount/empty-reason/unknown-staff rejection sweep for movements, 404s recording or listing movements against an unknown session id, a blind cash count recorded and confirmed via a follow-up `GET`, a four-way rejection sweep for counts (re-count, negative amount, unknown staff, closed session), 404 recording a count against an unknown session id, three real `admin` UI tests — opening and closing a session, recording a pay-out through the movements panel, and recording a count through the count panel then confirming it collapses to read-only — every test creates its own fresh terminal rather than sharing the seeded demo "Caixa 1", since only one session can be open per terminal at a time; PAY-11's variance report — the expected-amount computation from a float, a pay-in, a pay-out and a real cash payment, `countedAmount`/`variance` staying `null` until a count exists then correctly populated (a negative variance) both before and after the session closes, a card tender excluded from `totalCashPaymentsTaken`, a split payment's cash leg counted while its card leg isn't via the batch-level `cashSessionId`, a payment against an unknown session id rejected with the order left unpaid, a 404 on the variance endpoint itself for an unknown session id, and a real `admin` UI test recording a count and confirming the variance report and its badge appear immediately after — see [cash sessions](../features/cash-sessions.md)), `payments.spec.ts` (PAY-01/02/03/04/05/06 — a cash tender covering the total with correct change, a tender smaller than the total recorded as a valid partial payment whose balance tracks correctly across a settling second tender that itself overpays, zero/negative amounts and an unsupported method (`'Voucher'`, since `'Card'` is a real method now) and an unknown order each rejected with their own code, a further tender against an already-settled order rejected with `payment.already_settled`, a card tender covering the total exactly with zero change, a card partial payment tracked the same as cash, an overpaying card tender rejected with `payment.card_tender_exceeds_balance` then the exact remaining balance settling normally, a genuine cash+card split settling an order exactly with both rows confirmed via a follow-up `GET /payments`, an invalid tender partway through a split batch leaving zero rows persisted (proving the all-or-nothing atomicity), an empty split tenders array rejected, a tip attributed to a real staff member with the name resolved both on record and on list, an unattributed tip, a negative tip and an unknown attributed staff id each rejected with their own code, listing payments incl. 404 for an unknown order, recording a payment *after* close to prove close isn't a precondition, and five `pos` UI tests — one full cash tender, one two-tender partial-then-settle flow, one signing in then recording a tip auto-attributed to the signed-in staff member, one switching the payment-method `<select>` to Card and settling across a partial-then-exact pair (the history list's own "(Cartão)" label confirming the selected method actually reached the recorded payment), and one filling the split form's cash leg, watching the card leg auto-compute the remainder, and settling in one click — see [cash payments](../features/cash-payments.md)), `order-concurrency.spec.ts` (ORD-21/QA-08 — several genuinely concurrent `POST /orders/{id}/lines` requests against the same order confirm no lost or duplicated line either way, and any `409` that does land is always the well-formed `order.concurrently_modified` body; deliberately does not assert a conflict must occur — tried forcing one first at 8-way and then 40-way concurrency and it never landed in this environment, so the deterministic proof of the mechanism itself lives in `OrderConcurrencyIntegrationTests.cs` instead, see above), `feature-flags.spec.ts` (IDN-16 — a platform-specific override wins over the tenant's "all platforms" row on resolve, an unconfigured key defaults to disabled not enabled, re-setting the same key/platform updates in place rather than duplicating the row, an empty key rejected, and the `admin` "Feature flags" screen adds and toggles a flag through a real browser). Also incl. `floor-drag-drop.spec.ts` (FLR-03's canvas half — a real `page.mouse` drag past half a cell persists a grid-snapped position via `PUT /tables/{id}`, confirmed with `expect.poll`; a drag under half a cell snaps back with no request fired) and a regression test in `idempotency.spec.ts` for a real bug found while writing that spec: `IdempotencyMiddleware` used to call `WriteAsync` unconditionally, even for an empty (204) body, which Kestrel rejects outright and crashes the whole connection — a replayed `DELETE` now proves the API stays up across both the original and the cached-replay code path; see the CI row below for why this was very likely the cause of the previously-unexplained `e2e` job failures. Also incl. `admin-shell.spec.ts`/`admin-language-toggle.spec.ts`/`admin-menu-management.spec.ts`, WEB-09/10, `menu-import-excel.spec.ts` (CAT-17's Excel half — a real `.xlsx` built at test time via `exceljs`, never a committed binary fixture: 2 valid + 2 invalid rows plus a wholly-blank row import the same way the CSV spec already proves, an empty file/non-`.xlsx`/corrupt `.xlsx`/missing-header column each rejected with their own code, and the `admin` UI imports a real file end to end), `order-course-firing.spec.ts` (ORD-07/08/09 — firing a named course sends only its own unfired lines, firing all sends the rest, firing twice is a no-op, a voided line is never fired, a line with no course only ever fires with "fire all", an unrecognised course/closed order/unknown order all rejected with their own codes, and the `pos` UI fires a course through a real browser showing a "Sent" badge), `menu-item-image.spec.ts` (CAT-02 — a real uploaded PNG's URL is genuinely fetchable through `UseStaticFiles`, replacing a photo deletes the old file from disk, an empty/oversized/wrong-content-type file each rejected with its own code, and the `admin` upload/remove round trip through a real browser), `menu-item-schedule.spec.ts` (CAT-11), `menu-item-couvert.spec.ts` (CAT-12), `identity-organization-site-terminal.spec.ts` (IDN-01), `price-lists.spec.ts` (CAT-05), `combos.spec.ts` (CAT-10), `menu-item-scheduled-price.spec.ts` (CAT-16 — retrofitted to fast-forward the API's own clock past the scheduled instant via `X-Brasa-Test-Clock` instead of a real ~2s wait, once QA-04 shipped), `table-groups.spec.ts` (FLR-05), `tax-rules.spec.ts` (CAT-07/08), `floor-multi-level.spec.ts` (FLR-07), `floor-realtime.spec.ts` (API-16/17 — two real browser tabs, neither ever calls `reload()`, proving a live SignalR push and not a poll), `test-clock.spec.ts` (QA-04, proves the clock-override mechanism directly against `GET /ping`), `staff.spec.ts` (IDN-08/09 — PIN verify/lockout/reset at the API level plus a real `admin` UI round trip), `manager-authorization.spec.ts` (IDN-11 — the gate itself: a non-manager credential, an unknown manager id, a wrong PIN, and a full 5-strike lockout against an isolated manager so the shared demo one stays usable by every other spec), `floor-section-assignment.spec.ts` (FLR-06 — a plain Staff-role member assigned/cleared, unknown staff/room 404s, and the admin UI round trip), `staff-login.spec.ts` (WEB-07 — `pos`'s own sign-in: wrong-then-correct PIN, sign-out, cancelling the modal, a locked-out staff member showing disabled in the picker), and table-label/takeaway-default English assertions in `language-toggle.spec.ts`), all green — hit the QA-02 table-pool-exhaustion limitation again while iterating on `combos.spec.ts` (several partial runs during development left leftover open orders and dirty tables consuming all 16), recovered with the documented runbook (close leftover orders, clear dirty tables); the same flake resurfaced again during CAT-07/08 verification, unrelated to that task's own tests — a different, pre-existing UI spec each time depending on which table a concurrent worker happened to grab, cleanly passing in isolation both times, recovered the same way; and again during ORD-07/08/09 verification, this time landing on that very feature's own new UI test (`order-course-firing.spec.ts`'s "the pos UI fires a course" case) — same shape (`Target page, context or browser has been closed` after a 30s timeout waiting on a free table near the end of a 173-test run), clean in isolation, recovered by finishing the leftover open order and clearing both tables it left behind by hand. A clean single run doesn't need it across several consecutive full runs under real parallel load (2 workers) — that repetition is what surfaced and then proved the fix for the table-occupy race below (and, later, occasionally exhausted the original 8-table pool under back-to-back full runs — a QA-02 scaling limitation, mitigated by doubling the seeded pool to 16). That same repeated-run discipline is what caught the API-10 JSON-casing regression below before it reached a commit, and shaped the API-09 pagination test itself: a first version asserting exact page sizes flaked under concurrent specs sharing the dev database, fixed by walking the full cursor chain and asserting only what must hold regardless of noise from other tests. UI walking-skeleton through the real table picker (QA-05), the modifier picker (CAT-03/04), the pre-bill preview (ORD-18/19), per-line kitchen notes (ORD-06), course firing (ORD-07/08/09), table transfer (ORD-12), line transfer (ORD-13, API-level), order merge (ORD-14, API-level), split by item and by cover (ORD-16/17, API-level), takeaway orders (ORD-20), menu item description/allergens/photo (CAT-02), menu bulk CSV/Excel import (CAT-17), request-bill floor-plan signal (FLR-04), 86-ing a menu item (CAT-13), menu item repricing incl. the past-order-lines-immune-to-a-reprice invariant (CAT-19), menu category visibility (CAT-01), menu `ETag`/304 caching (API-10, now retry-tolerant of legitimate concurrent catalog mutations from sibling specs), idempotency replay — a retried close never double-issues a fiscal document (QA-11), client version negotiation (API-06/07), order-history cursor pagination (API-09), response compression incl. error bodies (API-11), accessibility scans (QA-14), API-level split-math sweep (QA-03), order history/search (ORD-22), language toggle + cookie persistence (WEB-13). Now run in CI on every push — see the CI row below for its own history. See [../development/e2e-testing.md](../development/e2e-testing.md) |
...
> title) stayed open at this point in the project's history — no job
> runner existed yet (OPS-10) — closed in a later session; see the
> Infrastructure table above for how. **Verified
...
> **Update (multi-floor support, FLR-07):** `Room` gained `FloorLevel: int`
> (default `0`, so every existing and seeded room needed no data
> migration) — `0` ground floor, positive above it, negative below (a
> basement or cave). Deliberately not a separate `Floor` entity: every
> field a floor needs — which rooms sit on it, which tables those rooms
> hold — is computable from the rooms that carry its level, the same "no
> entity where a plain field already says the same thing" call FLR-05's
> `Table.GroupId` already made rather than a `TableGroup` row. `POST`/
> `PUT /rooms` both carry it now (`POST` defaults to `0` when omitted,
> `PUT` requires it explicitly, since a rename/reorder that silently reset
> the floor back to ground would be a real, surprising data loss). Framed
> as a **display** concern, not an access one — nothing about a table's
> lifecycle, occupancy, or any other Floor invariant changes; the only
> thing a floor level does is decide whether a badge or heading renders at
> all. That framing decided the UI shape directly: a "Floor N" badge in
> `admin`'s room editor, and a floor heading grouping `pos`'s table
> picker, both stay entirely invisible until a tenant's own rooms actually
> span more than one level — every restaurant seeded so far is
> single-storey, so both screens render byte-for-byte as before until
> that day comes. Once ambiguity does exist, *every* room gets labelled,
> not just the odd one out — a bare, badge-less room would otherwise be a
> second, silent way to mean "ground floor," indistinguishable from "this
> screen doesn't support floors yet." **Verified live**:
> `floor-multi-level.spec.ts` — `floorLevel` round-trips through create,
> `GET /floor`, and update; omitting it on `POST` defaults to `0`;
> creating a second room on a different floor makes `admin`'s badge
> appear on *every* room, including the seeded ground-floor ones, not
> only the new one; editing a room's floor level through the real UI
> round-trips to the API; `pos`'s table picker renders both a "Floor 0"
> and a "Floor 1" heading with each room correctly grouped under its own,
> in a real browser, not just asserted from the API response.

> **Update (SignalR hub, this codebase's first realtime channel,
> API-16/17):** `FloorHub` at `/hubs/floor` broadcasts one payload-less
> `FloorChanged` signal whenever any table's state changes anywhere — the
> six call sites that actually change it (`ClearTableAsync`/
> `RequestBillAsync` in `FloorEndpoints.cs`; `OpenOrderAsync`/
> `TransferOrderAsync`/`MergeOrdersAsync`/`CloseOrderAsync` in
> `OrderEndpoints.cs`) each call the same `NotifyFloorChangedAsync`
> helper right after their own successful save, composing
> `IHubContext<FloorHub>` the same way these handlers already compose
> Floor and Ordering at the API layer. API-17's own "REST equivalent"
> requirement is satisfied by construction rather than a bolted-on check:
> the message carries no data at all, so a receiving client always
> re-fetches `GET /floor` — there is nothing in the push payload that
> could ever disagree with the REST response, because there is no
> payload. `pos` is the first subscriber (`connectFloorHub`,
> `@microsoft/signalr`, `withCredentials: false` since this codebase has
> no cookie auth to carry — hard rule 7), re-fetching on both the initial
> signal and on `onreconnected`, so a dropped connection never leaves it
> silently stale past the next reconnect. No per-tenant/per-terminal
> targeting yet — every connection sits in one `Clients.All` group, a
> named, deferred gap waiting on IDN-06/07 (terminal identity) to key a
> SignalR group by. Installing `@microsoft/signalr` from inside `pos/`
> rather than the npm workspace root left a stale Vite dependency-scan
> cache pointing at a pre-hoist `node_modules` layout — `pos` failed to
> render *any* screen at all until `node_modules/.vite` was cleared and
> the dev server restarted; not a code bug, but a real trap for whichever
> future task next adds a new frontend dependency from inside a workspace
> member instead of the root. **Verified live**: `floor-realtime.spec.ts`
> — two real, separate Playwright browser tabs, neither ever calling
> `reload()`: opening a table from the second tab flips it to `Occupied`
> in the first purely from the pushed signal, and clearing a dirty table
> flips it back to `Free` the same way, proving the push itself, not a
> disguised poll.

> **Update (fixed-clock control for E2E, QA-04):** revisited once CAT-16
> (scheduled price changes) and CAT-07/08 (tax rule effective dates)
> actually needed it — both had already shipped by waiting out a real
> ~2s wall-clock window (`test.slow()`) instead, since nothing before
> them was time-sensitive enough to justify building this speculatively
> — see `docs/development/e2e-testing.md`'s own honest "not built yet"
> note, now updated. `TestableClock : IClock` holds its override in a
> static `AsyncLocal<DateTimeOffset?>`, not a DI-scoped service:
> `MockFiscalProvider` is deliberately **singleton** (its in-memory
> sequential document numbering must survive across requests, not reset
> every time), and a singleton cannot consume a scoped `IClock` —
> ASP.NET Core's own DI validation refused to start the app on the first
> attempt, a real captive-dependency bug caught before it shipped
> (`Cannot consume scoped service 'IClock' from singleton
> 'IFiscalProvider'`), fixed by moving the override into an `AsyncLocal`
> instead, the exact same reason `TenantContextAccessor` already exists
> in this codebase rather than a plain scoped `TenantContext`.
> `TestClockMiddleware` reads an optional `X-Brasa-Test-Clock` header
> (ISO 8601) and fixes the override for that request's own async call
> chain only — ASP.NET Core runs each request on its own async flow, so
> two Playwright workers overriding to different instants against the
> one shared running API instance never interfere with each other.
> Mirrors `DevTenantMiddleware` exactly: registered unconditionally,
> throws on the very first request if `IsProduction()`, so a
> misconfigured deployment fails loudly rather than ever letting a
> client forge the instant a fiscal document is issued at. A missing or
> unparseable header is silently ignored — this is a testing lever, not
> public API surface, so there is nothing to reject with an error
> response. **Verified live**: `test-clock.spec.ts` proves the mechanism
> directly against `GET /ping` (which echoes `IClock.UtcNow` verbatim,
> independent of any feature's own business logic) — an override lands
> exactly on the request that sent it, a concurrent unheadered request
> still sees the real clock, and an unparseable header degrades to the
> real clock too. `menu-item-scheduled-price.spec.ts` (CAT-16) is the
> real first consumer, retrofitted to fast-forward past the scheduled
> price's effective date instead of a real wait — same assertions
> (not-yet-active, then active, then charged correctly by `AddLine`),
> now instant and deterministic every run instead of `test.slow()`.

> **Update (staff PIN accounts, IDN-08/09):** `Staff` (Identity module,
> site-scoped the same way `Terminal` already is) closes the "PIN
> hashing, lockout, and rotation policy" row and the PIN-*verification*
> half of "staff PIN sign-in" — not the "on a paired terminal" half,
> since no terminal pairing (IDN-07) exists to sign in *on*.
> `POST /staff/{id}/verify-pin` checks a PIN against a *known* staff id,
> deliberately not "identify me by PIN alone with no picker": without
> knowing which staff member is attempting, a failed PIN can't be
> attributed to the right person's own lockout counter without either
> skipping lockout tracking for that path entirely or incorrectly
> penalising every non-matching staff member on someone else's mistake.
> Hashing is PBKDF2-HMAC-SHA256 via .NET's own `Rfc2898DeriveBytes` (no
> new dependency, 210,000 iterations — OWASP's 2023 minimum, stored
> alongside each hash so a future increase never invalidates an
> already-issued PIN), verified with `CryptographicOperations.FixedTimeEquals`.
> Locks out after 5 consecutive incorrect PINs for 15 minutes — even a
> *correct* PIN is refused while locked, and `PUT /staff/{id}/pin`
> resets both the PIN and the lockout together, an admin action with no
> old-PIN check, the same "ships ahead of manager authorisation" shape
> every other admin mutation in this codebase already has. Deliberately
...
> gate yet" as their own named, deferred gap (IDN-11 is the eventual real
> gate). `admin`'s "Equipa" screen (closing WEB-11's staff half) is the
> last placeholder nav entry in that shell now live: list staff, add one
> with an initial PIN, reset a PIN — assumes the first organization's
> first site, since no site-selector exists anywhere in either client
> yet. Building `TestableClock` as a scoped `IClock` (QA-04) had already
> found the "singleton can't consume scoped" DI trap once; `Staff`'s own
> PIN hash column needed a different, smaller EF trick — a `private`
> auto-property mapped via `builder.Property<string>("PinHash")`
> (string-keyed shadow-style access works for genuinely private
> properties too, not just true shadow properties), so nothing outside
> `Staff` itself can ever read a hash back out. **Verified live**:
...
> resetting the PIN clears the lockout and the new PIN works
> immediately; an empty name, a malformed PIN (too short/long/non-digit),
> an unrecognised role and unknown site/staff ids are all rejected with
> their own codes; `admin`'s staff screen adds a staff member and resets
> their PIN through the real UI, both proven to actually take effect via
> a follow-up API call afterward, not just "the UI showed no error."

> **Update (manager authorisation, IDN-11):** The gate ORD-10/ORD-11 both
> shipped ahead of — `POST /orders/{id}/lines/{lineId}/void`,
> `PUT /orders/{id}/lines/{lineId}/discount` and `PUT /orders/{id}/discount`
> now all require `managerStaffId`/`managerPin` in the body.
> `OrderEndpoints.AuthorizeManagerAsync` composes `IdentityDbContext` into
> Ordering at the API layer — the same Catalog+Identity composition shape
> `PriceListEndpoints` already established for CAT-05 — and checks the
> credential names a real `StaffRole.Manager` *before* `Staff.VerifyPin` is
> ever called, so a Staff-role id offered here is rejected outright without
> spending any of that person's own lockout budget. Once role is confirmed,
> PIN verification reuses `Staff.VerifyPin` exactly as
> `POST /staff/{id}/verify-pin` (IDN-08/09) already does — same lockout
> mechanics, same "known id, not blind PIN entry" shape, and the identity
> `DbContext` is saved regardless of the verify outcome so a wrong guess
> still advances that manager's own `FailedPinAttempts`. A per-call
> credential, not a session: nothing here is cached or reused across
> requests — no terminal pairing or OAuth/JWT session (IDN-03…07) exists to
> hang a session off yet anyway. `identity.staff_not_manager` is the error
> registry's first real `ErrorType.Forbidden`/403 call site — the type has
> existed since API-03's original five, never constructed until now. No
> pos/admin UI prompts for a manager credential yet (no staff-picker exists
> in either client), so this ships the gate itself, the same "mechanism
> before the trigger" shape this codebase has used repeatedly (CAT-13/19,
> ORD-10/11 themselves). **Verified live**: `manager-authorization.spec.ts`
> — a non-manager staff credential is rejected (`identity.staff_not_manager`)
> without the line being touched, and the real manager then succeeds against
> the same line right after; an unknown `managerStaffId`
> (`identity.staff_not_found`) and a manager's own wrong PIN
> (`identity.pin_incorrect`) are both rejected, then the correct PIN still
> works; the same gate covers both line and order discounts, and role is
> checked before PIN (a non-manager's *wrong* PIN still surfaces as
> "not a manager," never "incorrect PIN"); 5 consecutive wrong PINs against
> an isolated manager (not the shared seeded "Ana Ferreira," so parallel
> specs relying on her aren't disturbed) lock them out
> (`identity.staff_locked`) even for their own correct PIN afterward,
> mirroring IDN-09's own lockout test exactly. `void-line.spec.ts` and
> `discounts.spec.ts` needed no test-logic changes at all — the shared test
> helpers (`support/api.ts`) default every void/discount call to the seeded
> demo manager's credentials unless a caller passes its own, so both specs
> keep testing exactly what they always tested (void/discount behaviour)
> while now genuinely exercising the real gate instead of no gate at all.

> **Update (waiter section assignment, FLR-06):** Unblocked the same
> session by IDN-08/09's `Staff` — `PUT /rooms/{id}/section` assigns or
> clears (`staffId: null`) which waiter is working a room as their
> section. A room was already the "which area" granularity a real
> *secção* means (Salão, Esplanada), so this is one new nullable field,
> `Room.AssignedStaffId` — a plain opaque `Guid?` reference to Identity's
> `Staff`, the same pattern `Order.TableId` already uses for a Floor
> `Table` — rather than a new entity, the same "no entity where a plain
> field says the same thing" call FLR-05's `Table.GroupId` and FLR-07's
> `Room.FloorLevel` already made. `FloorEndpoints` composes
> `IdentityDbContext` to confirm a non-null `staffId` names a real staff
> member before assigning (`404 identity.staff_not_found` otherwise) —
> the same Catalog/Ordering+Identity composition shape
> `PriceListEndpoints`/`OrderEndpoints` already use.
> `RoomDto.AssignedStaffName` resolves fresh from Identity on every
> `GET /floor`, batched into one query across every room rather than
> N+1, so a later-renamed staff member is never shown stale. Turned out
> to key off `Staff` directly rather than `Site`, unlike what IDN-01's
> own row once expected — a room has no site relationship of its own to
> match a section's staff id against. Any staff role works, Manager or
> plain Staff — unlike IDN-11 (shipped the same session), this isn't a
> privileged action, so it reuses none of that gate's machinery beyond
> the same "confirm the id names a real `Staff` row" existence check.
> `admin`'s room editor gets a section `<select>` next to each room;
> `pos` shows nothing yet. **Verified live**:
...
> **Update (pos staff sign-in, WEB-07):** Unblocked the same session by
> IDN-08/09's `Staff` — a "Sign in" button in `pos`'s header
> (`StaffLogin.tsx`) opens a staff-picker-then-PIN modal, calling the same
> `POST /staff/{id}/verify-pin` endpoint IDN-11's own manager-authorisation
> gate reuses — this is that mechanism's second real caller. Deliberately
> non-blocking: signing in gates nothing else in `pos` today. Building a
> real login *gate* would have meant updating every existing E2E spec that
> drives `pos` directly from a fresh `page.goto('/')` — the entire ordering
> flow's own test suite — a materially larger, riskier change than "a
> real, working sign-in exists," and the same "mechanism before the
> trigger" shape IDN-08/09 itself shipped in (nothing consumed its own
> verify-pin endpoint at all until IDN-11, several tasks later). A
> locked-out staff member appears in the picker but its button is
> disabled, so the UI never even lets a PIN be attempted against a locked
> account; a wrong PIN shows an inline error and leaves the picker on the
> same PIN-entry screen to retry (not back to the name list); success
> shows "Hi, {name}" and a sign-out control that returns to "Sign in".
> `pos` resolves the same "first organization's first site" site `admin`
> already assumes. No persistence across a reload — plain React state,
> since no session/token concept exists yet (IDN-03…05) to persist it
> into more durably; a page refresh signs out silently, an honest
> limitation given today's actual auth story
> (`DevTenantMiddleware`), not a bug. **Verified live**:
> `staff-login.spec.ts` — wrong PIN then correct PIN succeeding, sign-out
> returning to the sign-in button, cancelling the modal leaving the
> unsigned-in state untouched, and a staff member locked out via 5
> API-level wrong attempts showing disabled in the picker. Created
> directly on the real seeded demo site (never an isolated org/site,
> since `pos` only ever resolves that one site's staff) — the locked-out
> fixture is a freshly-created staff member there, never the shared
> "Ana Ferreira"/"Tiago Costa" every other spec depends on staying usable.
> Full suite green (64 backend, 163 E2E, two confirmed pre-existing QA-02
> table-pool flakes — a different unrelated spec each run, clean in
> isolation both times) — the pre-existing 160 all still pass unchanged,
> confirming the non-blocking design really doesn't disturb anything that
> never signs in.

> **Update (menu item photo upload, CAT-02):** the image-upload half of
> this row (previously 🚧 — "needs file storage infra") is built now,
> the same "no real infra credentials in this environment, skip it
> honestly" reasoning OPS-11 (deployment) already established, applied
> to storage instead: `MenuItemImageStorage` writes uploaded photos under
> `{ContentRoot}/uploads/menu-items/` on local disk, GUID-named
> (`Guid.CreateVersion7()`, never the caller's own filename), served
> back through a second `app.UseStaticFiles` mount at
> `/uploads/menu-items/…` — deliberately outside `/api/v1`, since it's a
> file, not a JSON resource. `POST /menu/items/{id}/image` (multipart,
> field `file`, JPEG/PNG/WebP, 5MB max) saves the new file and persists
> the new `MenuItem.ImageUrl` before ever deleting whichever file it
> replaces, so a failed upload degrades to "still has the old photo,"
> never "has no photo" — the same before-you-delete ordering
> `Delete`/`SetImageUrl` follow on the remove side too.
> `DELETE /menu/items/{id}/image` clears it; a never-set image is a
> no-op, not an error. Because `MenuItemDto.ImageUrl` is a path relative
> to the **API's own origin**, not `/api/v1` like every other field, both
> `pos` and `admin` gained a small `apiOrigin` export (`API_BASE_URL`'s
> own origin) so `<img src>` resolves correctly instead of 404ing
> against `/api/v1/uploads/…`. Building it surfaced a real, non-obvious
> gotcha, caught live by the E2E suite rather than at compile time or by
> `dotnet build`: ASP.NET Core auto-attaches antiforgery
> request-validation metadata to *any* Minimal API endpoint binding an
> `IFormFile`, as a default CSRF mitigation for form posts, regardless of
> whether the app registers antiforgery services at all — this one never
> does (hard rule 7: no cookie auth). The first live upload attempt threw
> `InvalidOperationException: ... contains anti-forgery metadata, but a
> middleware was not found that supports anti-forgery` — a 500 with no
> hint from the type system, since `.Accepts<IFormFile>()` compiles fine
> and the failure only manifests once a real multipart request hits the
> route. Fixed with `.DisableAntiforgery()` on the route mapping, the
> correct call for an endpoint that was never protected by antiforgery in
> the first place. `admin`'s `MenuManager` renders a thumbnail + remove
> button when a photo is set, or a styled upload control — a clickable
> label wrapping a `display:none` native `<input type="file">` — when
> none is; `pos`'s `MenuGrid` renders the same thumbnail read-only.
> Both apps' shared `client.ts` `request()` helper had a latent bug this
> feature would have hit in production, caught and fixed before it
> shipped rather than by a failing test: it unconditionally set
> `Content-Type: application/json` whenever any request body was present,
> which would have corrupted a multipart upload (the browser must set its
> own boundary) — fixed by skipping the header when `init?.body
> instanceof FormData`. **Verified live**: `menu-item-image.spec.ts` — a
> real uploaded PNG's returned URL is genuinely fetchable through
> `UseStaticFiles` and serves the right `Content-Type`, not just present
> in the DTO; replacing a photo deletes the old file from disk (its old
> URL 404s afterward, the new one 200s); an empty file, a >5MB file, and
> a disallowed content type are each rejected with their own code
> (`catalog.image_required`/`catalog.image_too_large`/`catalog.invalid_image_type`),
> alongside an unknown item (`catalog.item_not_found`); removing a
> never-set image is a no-op; the `admin` UI uploads and removes a real
> file through the real browser, confirmed via a follow-up API call
> rather than trusting the DOM alone. Full suite green (64 backend, 168
> E2E, one confirmed pre-existing QA-02 table-pool flake — a different
> spec than any of this feature's own, clean in isolation) — the
> pre-existing 163 all still pass unchanged.

> **Update (course firing, ORD-07/08/09):** `Course.cs`'s own doc comment
> named this exact task as its future consumer before it existed
> ("Course *firing* (ORD-07) isn't built yet; this is the tag it will
> read from once it is") — CAT-14's `MenuItem.Course` (`Starter`/`Main`/
> `Dessert`/`Drink`) finally gets a real reader. `OrderLine.Course` is a
> plain string snapshotted from the catalog item at add-time, the same
> `ItemName`/price/VAT-rate convention every other line field already
> uses, not a live join — Ordering still never references Catalog's
> `Course` enum directly (module boundaries); the API layer resolves and
> validates the name before it ever reaches `Order.AddLine`.
> `Order.FireLines(course, firedAtUtc)` is the one new domain method
> behind `POST /orders/{id}/fire`: a specific course name fires only its
> own unfired, non-voided lines (ORD-08's "partial" send), `course: null`
> fires everything still pending regardless of course (the "full" send).
> Idempotent by construction — an already-fired line is simply skipped,
> firing the same course twice is a no-op, not an error. `OrderLine.
> IsFired`/`FiredAtUtc` (ORD-09) is deliberately the only status this
> slice tracks: richer states (`Preparing`/`Ready`/`Served`) need a KDS to
> transition them, which doesn't exist yet (KIT-10…13 are I4) — the same
> "ship only what the current seam can actually use" discipline CAT-11/16
> already followed for scheduling. No manager authorisation, unlike void
> (ORD-10) and discounts (ORD-11) — firing changes nothing financial, so
> gating it the same way would just be friction with no fiscal reason
> behind it. `pos`'s `OrderSummary` gets a `fire-controls` bar — one
> button per course with an unfired line still on the order, plus a
> "Fire all" — that disappears entirely once nothing is left unfired; a
> fired line shows a small "Sent" badge next to its name. **Verified
> live**: `order-course-firing.spec.ts` — firing a named course leaves a
> different course's own line untouched, firing all then sends the rest;
> a voided line is never fired even by "fire all"; a line with no course
> assigned fires only with "fire all," never a named course (there is no
> course value it could ever match); an unrecognised course name, a
> closed order and an unknown order id are each rejected with their own
> code (`order.invalid_course`/`order.not_open`/`order.not_found`); and
> the `pos` UI fires a course through a real browser, the badge appearing
> and the fire-controls bar disappearing once nothing is left pending.
> Full suite green (64 backend, 173 E2E, one confirmed pre-existing
> QA-02 table-pool flake — this run it landed on this feature's own new
> UI test, same failure shape as every prior recurrence, clean in
> isolation) — the pre-existing 168 all still pass unchanged.

> **Update (menu bulk import — Excel, CAT-17's second half):** the
> row's own remaining gap ("Excel not built") is closed —
> `POST /menu/items/import/excel` accepts a real `.xlsx` file
> (`ExcelDataReader`) and shares the exact same `ImportRowsAsync`
> row-processing pipeline the CSV endpoint (`POST /menu/items/import`)
> already used, once `ExcelImportParser` turns the first worksheet into
> the identical row shape `CsvParser` produces. Every validation, every
> error code, and the per-row-independence guarantee now apply to both
> formats from one implementation, not two. `admin`'s single import
> control routes by file extension rather than gaining a second one.
> Two problems surfaced along the way, both about dependencies, neither
> about the import logic itself: first, `ClosedXML` (the first library
> tried) pulls in `SSH.NET` 2025.1.0 transitively, which the
> zero-warning build gate correctly refused — switched to
> `ExcelDataReader`, chosen specifically because reading (never writing)
> Excel needs a much smaller dependency footprint. Second, and unrelated
> to either library: NuGet's live vulnerability advisory feed flagged
> that same `SSH.NET` version in `Testcontainers` (present all session,
> an optional remote-Docker-over-SSH dependency) between one
> `dotnet build` and the next, with zero local code changes — confirmed
> genuinely external by reverting every change and rebuilding a
> byte-identical tree, still broken. Fixed by pinning a direct `SSH.NET`
> reference to a patched version in `Brasa.Api.IntegrationTests`
> (NuGet treats a dependency's unbracketed version as a floor, not an
> exact pin) — see the two new `docs/ai/README.md` §7 trap entries.
> A third, Excel-specific gotcha: `ExcelDataReader` throws
> `NotSupportedException: No data is available for encoding 1252` on
> every `.xlsx` (not just legacy `.xls`) until
> `Encoding.RegisterProvider(CodePagesEncodingProvider.Instance)` is
> registered — .NET Core dropped that codepage by default — fixed with
> the first-party `System.Text.Encoding.CodePages` package and a static
> constructor on `ExcelImportParser`, which also tripped NuGet's package-
> pruning warning (`NU1510`) since the SDK considers it redundant with
> the shared framework's reference assembly; suppressed narrowly via
> `NoWarn="NU1510"` on that one `PackageReference`, with a written reason,
> since the runtime provider genuinely isn't present without it.
> **Verified live**: `menu-import-excel.spec.ts` — 2 valid + 2 invalid
> rows plus a wholly-blank row in one real `.xlsx` (built at test time
> via `exceljs`, a devDependency of `e2e` only, never a committed binary
> fixture) → `created: 2`, the same two row-level errors the CSV spec
> already proves, the blank row silently skipped rather than reported;
> an empty file, a non-`.xlsx` file, a genuinely corrupt `.xlsx`, and a
> header missing a required column each `400` with their own code; the
> `admin` UI imports a real `.xlsx` end to end. The pre-existing CSV
> spec re-verified unchanged, confirming the shared-pipeline refactor
> didn't disturb it. Full suite green (64 backend, 176 E2E, no flakes
> this run) — the pre-existing 173 all still pass unchanged.
...
- **The E2E suite's own repeatability.** 32 tables are seeded (doubled twice:
  8→16 once the suite passed twenty tests, 16→32 once it passed 180 —
  back-to-back full runs started occasionally exhausting the smaller pool
  both times) and the dev database isn't reset between runs — every spec
  that opens one now closes and clears it.
...
| CI (GitHub Actions) | 🚧 | Runs on every push to `main`. Build-and-test and vulnerable-package-scan jobs consistently green. `openapi-drift` caught real drift three times running early on (CAT-02, ORD-07/08/09, CAT-17 each shipped an endpoint without the documented regeneration step), then failed on **every push after that, including a docs-only commit** — not real drift, a formatting bug in the regeneration command itself (`ConvertTo-Json`'s indentation never matches CI's own `jq`-based check byte-for-byte, confirmed by deep-sorting both documents to primitives and diffing that instead of raw text, which showed zero semantic difference). Fixed by moving regeneration to Node (`infra/scripts/regenerate-openapi.mjs`) — not yet reconfirmed by an actual CI run, since this environment cannot push; verified instead by literally replicating CI's own `jq 'del(.servers)'` transformation and confirming byte-identical output. The `e2e` job separately crashed outright on `pos`'s (and `admin`'s) Playwright `webServer` failing to start (`vite@8`'s new hard `rolldown` dependency hitting a real, documented npm bug installing its Linux native binding, `npm/cli#4828`, still-open sibling `npm/cli#8320`) — an explicit `npm install -g npm@latest` step alone did **not** hold up on a real run, so the actual fix stops trusting npm's optional-dependency resolution for the platform binary at all and installs the exact `@rolldown/binding-linux-x64-gnu` version each lockfile already pins rolldown to as an explicit, `--no-save`, CI-only step — also not yet reconfirmed by a real run. The `IdempotencyMiddleware` 204-crash bug that caused this job's own earlier, previously-undiagnosed red runs (any DELETE endpoint, cascading into whatever ran next in a busy full-suite run) is fixed and **was** reconfirmed live locally. See the two new trap entries in [docs/ai/README.md](../ai/README.md) for the full reasoning behind both newest fixes |
...
| Database backup + restore drill (OPS-12) | ✅ | `infra/scripts/{backup-database,restore-database,restore-drill}.ps1` — `pg_dump`/`pg_restore` via `docker exec`+`docker cp` (never a PowerShell text redirect, which would corrupt the binary dump format), restoring into a scratch database and comparing every table's row count across every schema except `hangfire.*` (its lock/heartbeat tables mutate continuously, independent of the drill's own timing — not tenant data needing a disaster-recovery guarantee). Now scheduled too: `DatabaseBackupJob` (OPS-10, Hangfire) runs a nightly backup and a weekly full drill, never in Production. See [backup-and-restore.md](../development/backup-and-restore.md). **Verified live**: both jobs manually triggered end to end through the running Hangfire server — a real ~1MB backup file produced, a real drill passing clean (23 tables, all row counts matched); both failure paths (a count mismatch, a table missing entirely) were deliberately reproduced against scratch databases and correctly flagged, not just assumed correct from the code |
| Hangfire (OPS-10) | ✅ | `Hangfire.AspNetCore`/`Hangfire.PostgreSql` on the migrations connection (its storage schema needs DDL rights `brasa_app` doesn't have). Dashboard at `/hangfire`, outside Production only — no auth exists yet to gate it behind. Schedules `DatabaseBackupJob`'s two recurring jobs (OPS-12). **Verified live**: dashboard reachable, both recurring jobs registered and manually triggered, each producing a real, correct result |
| Load testing (QA-13) | 🚧 | `src/web/e2e/load/{read-load,write-load,run-all}.mjs` (`npm run load`) — read path via `autocannon`, write path (open→add line→close→clear) via a hand-rolled correlated-flow harness bounded by the seeded table pool (32 as of this writing). Scoped honestly against what this row's own title can't yet mean: no multi-tenant data exists for "50 sites," and `Reporting` is empty so "reporting queries never touch the transactional path" has nothing to test. See [load-testing.md](../development/load-testing.md). **Verified live**: read path p95 199ms (menu) / 112ms (floor) at 20 concurrent connections; write path zero failures across 2,204 mutating requests at 10 concurrent terminals, every operation p95 ≤132ms. Found a real ~40x latency trap along the way, not an app bug: `Debug`-level Serilog logging (the `appsettings.Development.json` default) inflated `GET /menu` from p50 67ms to p50 2583ms purely from logging I/O under concurrency — confirmed by a fast single sequential `curl` at the same log level, which ruled out the query itself, then by re-measuring at `Information` (production's own default) and seeing the ~40x drop |
