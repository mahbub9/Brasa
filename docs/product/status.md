...
| `kds` | ⬜ | |
| `admin` | 🚧 WEB-09 shell | React 19 + Vite 8 + TS, same tooling as `pos`, on port 5174. One live screen ("Visão geral"): real category/item/room/table counts from `GET /menu` and `GET /floor`, so the shell is provably wired to the API and not a static mock. Menu/Floor/Staff nav entries are labelled "Brevemente" rather than silently missing — WEB-10/11 build the actual editors. No i18n toggle (pt-only static copy) and no auth yet (depends on IDN) |
