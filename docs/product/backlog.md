...
| API-13 | OpenAPI document generation, committed to the repo | ✅ [docs/openapi/v1.json](../openapi/v1.json), generated by `Microsoft.AspNetCore.OpenApi` (already wired for the dev-only Swagger-style UI) and committed so the API's shape is reviewable in a diff. Regenerated by hand for now — CI enforcement that it hasn't drifted is API-14, deliberately not built yet. **Known gap, found while building API-15:** every response body is undescribed — every endpoint returns a bare `Results.Ok(...)`/`IResult`, and ASP.NET Core's OpenAPI reflection can only infer a *request* body's shape that way, never a response's. Request bodies, path and query parameters are correctly described; nothing a client receives back is, and has never been since this row first shipped. See [docs/openapi/README.md](../openapi/README.md) |
...
| API-15 | TypeScript SDK generation into `web/sdk` | 🚧 A new `@brasa/sdk` workspace package (`npm run generate`) runs `openapi-typescript` against the committed `docs/openapi/v1.json`, producing `src/schema.ts` — every request body, path and query parameter, correctly typed and verified against real endpoints (`src/schema.guard.ts`, a permanent type-level check, not a throwaway). Building this surfaced a real, previously-unnoticed gap in API-13 itself: every response body has always been undescribed (see that row's own update) — not something this task broke, but something it needed and found missing. Not yet consumed anywhere — no `pos`/`admin` call site switched over, and there's no typed fetch client (e.g. `openapi-fetch`) wrapping these types yet, since this codebase's own `ApiError`/`ProblemDetails` handling doesn't map onto a generic wrapper without its own dedicated design pass. See [src/web/sdk/README.md](https://github.com/mahbub9/Brasa/blob/main/src/web/sdk/README.md) for the exact scope |
...
| WEB-03 | Shared `web/sdk` — OpenAPI-generated typed client | 🚧 The same underlying deliverable as API-15 (this epic's own row predates that one and named the same package first) — see that row for what's actually built: generated request/path/query types, verified against real endpoints, not yet a typed *client* this row's own title promises, and not yet consumed by `pos`'s hand-written `src/api/` (still the I0 placeholder) or `admin`'s |
...
| OPS-12 | Automated database backup and a tested restore drill | 🚧 The mechanism and the drill both exist and are proven live — `infra/scripts/backup-database.ps1`/`restore-database.ps1`/`restore-drill.ps1`, `pg_dump`/`pg_restore` via `docker exec`+`docker cp` (never a PowerShell text redirect — binary corruption risk), restoring into a scratch database and comparing every table's row count across every schema. "Automated" is the open half: nothing schedules it yet, since there's no production deployment to schedule it in (OPS-11) and no job runner (OPS-10). **Verified live**: a real drill against the dev database passed clean (12 tables, all row counts matched); both failure paths (a row-count mismatch, a table missing entirely) were deliberately tested against scratch databases too, not just assumed correct by reading the code. See [backup-and-restore.md](../development/backup-and-restore.md) |
