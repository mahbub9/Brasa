...
| ID | Task | Status |
|---|---|---|
| PAY-01 | Payment / tender model | ✅ New `Brasa.Modules.Payments` module — a `Payment` entity (`OrderId`, `Method`, `AmountDue`, `AmountTendered`, computed `AmountApplied`/`Change`/`RemainingBalance`, `PaidAtUtc`), its own `payments` schema, own `PaymentsDbContext`/migration/RLS. Purely additive: does not gate `Order.Close()` and is never queried by Ordering — the API layer composes both contexts, the same shape `PriceListEndpoints` already uses. `AmountDue` is a remaining balance, not always the order's full total — see PAY-05. See [cash payments](../features/cash-payments.md) |
| PAY-02 | Cash tender with change calculation | ✅ `PaymentMethod.Cash` is the only case today. `POST /orders/{id}/payments` reads the order's own remaining balance server-side (never trusts a client-sent amount due), validates the tender, and returns the computed `change`. `GET /orders/{id}/payments` lists every payment recorded. `pos`'s receipt screen (`CashPayment.tsx`) records a tender and shows change due. See [cash payments](../features/cash-payments.md) |
| PAY-03 | Card tender, manually captured from a standalone TPA | ✅ `PaymentMethod.Card` — this codebase never talks to a card processor, staff key in the amount a real standalone TPA already charged, the same "record what happened, don't drive the hardware" shape `Cash` already has. No schema change needed — `Method` was already a plain `character varying(20)` string column. The one new domain rule: a card tender cannot exceed the balance (`400 payment.card_tender_exceeds_balance`), since a TPA has no change to give back, mirrored in both `Payment`'s constructor and `RecordPaymentAsync` so the API returns a proper `Result`/`Error` rather than letting the domain `ArgumentException` bubble; a card tender smaller than the balance is still a valid partial payment (PAY-05), same as cash. `pos`'s `CashPayment.tsx` gets a payment-method `<select>` (Cash/Card, `@brasa/ui`'s `SelectField`), defaulting to Cash and persisting across however many tenders it takes to settle; the history list now names each tender's method. **Verified live**: backend suite (101 tests) + `verify.ps1` clean, no OpenAPI drift beyond the endpoint summary text (the wire shape was always a plain string, so no schema widening was needed); `payments.spec.ts` grew to 13 tests (a card tender covering the total exactly with zero change, a card partial payment, an overpaying card tender rejected then the exact remaining balance settling normally, plus the pre-existing "unsupported method" test switched off `'Card'` since it's supported now) all pass alongside the full E2E suite (201 tests, one pre-existing QA-02 table-pool flake in an unrelated spec, clean in isolation). See [cash payments](../features/cash-payments.md) |
| PAY-04 | Split tender across multiple methods | ✅ `POST /orders/{id}/payments/split` records several tenders — any mix of `Cash`/`Card` — against an order's remaining balance as one atomic action: each tender in the request body applies in order against what's still owed after the ones before it in the same batch, and if any tender is rejected (an invalid amount, an overpaying card tender, or the balance reaching zero before every tender is consumed), the whole batch is rejected and nothing is persisted. Atomic by construction, not an explicit `BeginTransactionAsync` — nothing is added to `PaymentsDbContext` until every tender in the batch has validated successfully, so a single `SaveChangesAsync` call (already one implicit EF Core transaction) either writes every row or none. A shared `BuildTenderAsync` helper, extracted from the single-tender endpoint's own validation, is now the one place both endpoints apply the method/amount/tip/card-no-overpay/staff-attribution rules, so they can't drift apart. Deliberately no tip/staff fields on a split — attributing a tip to one portion of a split is ambiguous ("which tender earned it?"), so a tip stays a single-tender-only concept (PAY-06). `pos`'s `CashPayment.tsx` gets a "Split between cash and card" toggle revealing two linked amount fields — typing one auto-fills the other with whatever's left of the balance — that submit together as one click. **Verified live**: backend suite (101 tests) + `verify.ps1` clean, real OpenAPI additive drift for the new route regenerated and typechecked through `web/sdk`; `payments.spec.ts` grew to 16 tests (a genuine cash+card split settling an order exactly, confirmed via a follow-up `GET /payments` that both rows really landed; an invalid tender partway through a batch leaving zero rows persisted, proving the atomicity; an empty tenders array rejected; and a UI test filling the cash leg, watching the card leg auto-compute, and settling in one click) all pass alongside the full E2E suite (204 tests, no flakes this run). See [cash payments](../features/cash-payments.md) |
| PAY-05 | Partial payment against an open order | ✅ `Payment`'s constructor no longer requires `amountTendered` to cover `amountDue` — a tender smaller than what's owed is a valid partial payment (`AmountApplied` equals the tender, `RemainingBalance` stays positive). `POST /orders/{id}/payments` computes the order's remaining balance itself (its own total minus every prior payment's `AmountApplied`) and passes that as the new payment's `amountDue`, so a second or third tender only needs to cover what's left; rejects with `400 payment.already_settled` once the balance is already zero. Closes the "nothing stops a double payment" gap [cash-payments.md](../features/cash-payments.md) originally left open. `pos`'s `CashPayment.tsx` now accumulates recorded tenders and keeps its form open, showing the running balance, until the order is fully settled. **Verified live**: backend suite (101 tests) + `verify.ps1` clean, no OpenAPI drift beyond the documented `PaymentDto` field additions; `payments.spec.ts` (7 tests, up from 6: partial-then-settle tracking, already-settled rejection, a UI test recording two tenders through the receipt screen) all pass alongside the full E2E suite. See [cash payments](../features/cash-payments.md) |
| PAY-06 | Tips — recording and attribution | ✅ `Payment` grows `TipAmount` (real persisted column, defaults zero — new `AddTipToPayment` migration, hand-edited so its default currency string deserializes existing rows correctly, see the trap in `docs/ai/README.md`) and `AttributedStaffId` (nullable, a plain opaque reference to Identity's `Staff`, same convention `Room.AssignedStaffId` uses). A tip is entirely separate from the bill — never affects `AmountDue`/`AmountApplied`/`RemainingBalance` — and rides along on the same `Payment` row as its tender rather than its own row. Attribution is optional even when a tip is given (an unattributed tip goes to a shared pool); the constructor only rejects attributing a tip that isn't positive. `POST /orders/{id}/payments` validates `tipAmount >= 0` (`400 payment.invalid_tip_amount`) and, when a `staffId` is given, confirms it names a real `Staff` (reusing `404 identity.staff_not_found`, the same check `FloorEndpoints.AssignRoomSectionAsync` already does for FLR-06) before constructing the payment — now composing three modules (Payments+Ordering+Identity) at the API layer. `GET /orders/{id}/payments` batch-resolves every distinct `AttributedStaffId` in one query, the same shape `FloorEndpoints.ResolveStaffNamesAsync` uses for `RoomDto.AssignedStaffName`. `pos`'s `CashPayment.tsx` gets an optional tip field on every tender, auto-attributed to whoever is signed in (WEB-07) — no staff picker, since one already exists in the header. **Verified live**: backend suite + `verify.ps1` clean; `payments.spec.ts` (10 tests, up from 7: a tip attributed to a real staff member with the name resolved both on record and on list, an unattributed tip, a negative tip rejected, an unknown staff id rejected, and a UI test signing in then recording an auto-attributed tip) all pass alongside the full E2E suite. See [cash payments](../features/cash-payments.md) |
| PAY-07 | Refunds via credit note | ⬜ |
| PAY-08 | Cash session — *abertura de caixa* with float | ✅ `CashSession` (Payments module, new `cash_sessions` table) — a staff member's starting cash-float declaration against a `Terminal` (Identity), `OpenedByStaffId`/`TerminalId` both plain opaque references, the same convention `Payment.OrderId` uses. Purely a record — no payment endpoint requires an open session to exist yet, the same "mechanism before the trigger" shape this codebase already uses everywhere. Only one open session per terminal at a time, enforced at the API layer (`409 cash_session.already_open`) before a second row is ever constructed. `POST /cash-sessions` opens one (validates the terminal and staff are real, the float isn't negative); `POST /cash-sessions/{id}/close` is a bare status flip (`400 cash_session.already_closed` if already closed) — a blind count (PAY-10, since shipped the same week) and variance report (PAY-11) were this row's own later task, not this; `GET /cash-sessions/{id}` fetches one; `GET /terminals/{id}/cash-sessions/current` resolves the currently-open one for a terminal, `204 No Content` if none — not `200` with a `null` body, since `Results.Ok<T>(null)` writes zero bytes for the body, not the JSON literal `"null"`, a real trap found live via the E2E suite's own `response.json()` throwing on it (see the trap in `docs/ai/README.md`). `admin` gets a new "Cash sessions" nav entry — one row per terminal registered at the site, each showing open/closed status and an open (staff picker + float) or close action. **Verified live**: a real Postgres smoke test (open → GET current → duplicate rejected 409 → close → GET current 204 → reopen) run by hand against the live API before the E2E suite even existed for this feature, which is also how the `Results.Ok<T>(null)` trap was caught, before it could reach a commit; backend suite (101 tests) + `verify.ps1` clean (build, OpenAPI drift — a genuinely new set of routes — breaking-change check, vulnerable-package scan); `cash-sessions.spec.ts` (6 tests: open-then-current, duplicate-open rejected, close-then-current-is-204-then-reopen, negative float/unknown terminal/unknown staff each rejected, 404s on an unknown session id and terminal id, and a real `admin` UI test opening and closing a session through the real browser) all pass alongside the full E2E suite (210 tests, three pre-existing QA-02 table-pool flakes in unrelated specs across two runs, confirmed clean in isolation both times). No client UI in `pos` — a terminal opening its own shift session is a real future trigger, not built yet, the same "mechanism before trigger" gap as everywhere else this shape appears. |
| PAY-09 | Cash movements — pay-in, pay-out, with reason | ✅ `CashMovement` (Payments module, new `cash_movements` table in the existing `payments` schema) — a pay-in or pay-out against an open `CashSession`, always requiring a reason, the same "never silent" instinct `Order.VoidLine` already applies to voiding a line. `CashSessionId`/`RecordedByStaffId` are both plain opaque references, the same convention `CashSession.OpenedByStaffId` uses. `POST /cash-sessions/{id}/movements` validates the session is real and still open (`400 cash_movement.session_closed` otherwise), the direction parses to `PayIn`/`PayOut` (`400 cash_movement.invalid_direction`), the amount is positive (`400 cash_movement.invalid_amount`), the reason isn't empty (`400 cash_movement.reason_required`), and the staff id is real; `GET /cash-sessions/{id}/movements` lists them oldest-first, batch-resolving staff names the same way `PaymentEndpoints.GetPaymentsAsync` already does for tip attribution. `admin`'s "Cash sessions" screen grows a movements panel under each open session — a running list plus an add (direction/amount/reason/staff) form, visible only while that session is open. Generated the migration for real via `dotnet ef migrations add` again — this time getting the `EnableFor`/`EnableSystemReadFor` argument order right the first time by checking the actual signature rather than by analogy, the fix for the exact trap PAY-08's own migration hit. **Verified live**: a real Postgres smoke test by hand (open a session, record a pay-out, list it, close the session, confirm a further movement is rejected, confirm validation rejections) before `cash-sessions.spec.ts` grew its PAY-09 tests; backend suite (101 tests) + `verify.ps1` clean; the same spec file grew to 10 tests (a pay-out then a pay-in listed oldest-first with names resolved, a closed-session/invalid-direction/non-positive-amount/empty-reason/unknown-staff rejection sweep, 404s on an unknown session id for both record and list, and a real `admin` UI test recording a pay-out through the movements panel) all pass alongside the full E2E suite (214 tests, one pre-existing unrelated flake confirmed clean in isolation). See [cash sessions](../features/cash-sessions.md) |
| PAY-10 | Blind cash count | ✅ Three nullable fields added to `CashSession` itself (`CountedAmount`/`CountedByStaffId`/`CountedAtUtc`) rather than a new table — a session has at most one count, the same "no entity where a plain field says the same thing" call `ClosedAtUtc` already made for closing. "Blind" because nothing in this codebase shows an expected total to compare against first — no variance calculation exists yet (that's PAY-11). `POST /cash-sessions/{id}/count` validates the session is real, the staff id is real, and calls `CashSession.RecordCount` — a `Result`-returning domain method (not a throwing constructor) rejecting a negative amount (`400 cash_session.invalid_counted_amount`), a closed session (`400 cash_session.already_closed`), or a second count on the same session (`400 cash_session.already_counted`, since the entire value of a blind count is that it wasn't influenced by an earlier attempt). `admin`'s "Cash sessions" screen grows a count panel next to the movements one — a "Record blind count" button/form that, once submitted, collapses to a read-only line; there's no edit path, matching the domain's own "at most once" rule. Generated the migration for real via `dotnet ef migrations add` a third time — this one only adds nullable columns to the already-RLS-enabled `cash_sessions` table, so no `EnableFor` calls were needed at all (nothing new to grant access to). **Verified live**: a real Postgres smoke test by hand (open, count, re-count rejected, negative amount rejected, count-after-close rejected) before `cash-sessions.spec.ts` grew its own PAY-10 cases; backend suite (101 tests) + `verify.ps1` clean, real additive OpenAPI drift for the new route regenerated and typechecked through `web/sdk`; the same spec file grew to 14 tests (recording a count and confirming both the response and a follow-up `GET` reflect it, a four-way rejection sweep — re-count, negative amount, unknown staff, closed session — and a real `admin` UI test recording a count and confirming the form collapses to read-only) all pass alongside the full E2E suite (218 tests, one pre-existing unrelated flake confirmed clean in isolation). See [cash sessions](../features/cash-sessions.md) |
| PAY-11 | *Fecho de caixa* with variance reporting | ✅ Closed the gap PAY-10's own docs flagged as its sharper starting point: `Payment` gained an optional, nullable `CashSessionId` (Payments module, ordinary foreign key with an index — not an opaque cross-module reference, since `CashSession` already lives in this same module), threaded through `POST /orders/{id}/payments` and `.../payments/split` (the split endpoint takes one `cashSessionId` for the whole batch, not per-tender — a split happens at one terminal at one moment). Unvalidated against the session's own open/closed state deliberately (a payment predates the concept of a session for flows that never open one) but rejected with `404 cash_session.not_found` if the id given doesn't resolve to a real session. `GET /cash-sessions/{id}/variance` computes — never stores — `expectedAmount` (opening float + pay-ins − pay-outs + cash payments taken under that session; only `PaymentMethod.Cash` counts, a card tender never touches the drawer) and, once a blind count (PAY-10) exists, `variance` (counted − expected); both `countedAmount`/`variance` stay `null` until a count is recorded, and the whole report stays computable after the session closes since nothing gates further payments landing against a closed session's id. `admin`'s "Cash sessions" screen shows the report automatically under the blind-count panel the moment a count is recorded, gated on `session.countedAmount` rather than open/closed status. **Verified live**: a real Postgres smoke test by hand (opened a session with a 50.00 float, recorded a 20.00 pay-in and a 5.00 pay-out, rang up and paid a real 10.00 cash order tagged with the session, confirmed `expectedAmount` = 75.00 with `countedAmount`/`variance` null, recorded a 73.00 count and confirmed `variance` = -2.00 both before and after closing the session, confirmed a card tender leaves `totalCashPaymentsTaken` untouched, and confirmed a payment naming an unknown `cashSessionId` is rejected with `404` and leaves the order's balance untouched) before regenerating `docs/openapi/v1.json`/`web/sdk`; backend suite (101 tests) + `verify.ps1` clean (build, OpenAPI drift, breaking-change check, vulnerable-package scan); `cash-sessions.spec.ts` grew to 20 tests (6 new: the expected-amount computation and null-then-populated variance across a count and a close, a card tender excluded from `totalCashPaymentsTaken`, a split payment's cash leg counted while its card leg isn't, a payment against an unknown session id rejected with the order left unpaid, a 404 on the variance endpoint for an unknown session id, and a real `admin` UI test recording a count and confirming the variance report and its badge appear immediately after) all pass alongside the full E2E suite (224 tests, two pre-existing unrelated QA-02 flakes — one of them PAY-08's own admin-UI test, again — confirmed clean in isolation). See [cash sessions](../features/cash-sessions.md) and [cash payments](../features/cash-payments.md). |
